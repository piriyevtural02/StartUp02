...
const PlanCard = ({
  ...
  return (
    ...
    >
      ...
      <div className="p-8">
        <h3 className={`text-2xl font-bold mb-2 ${highlighted ? 'text-white' : 'text-gray-800 dark:text-white'}`}>
          {title}
        </h3>
        <div className="mb-4">
          <span className={`text-4xl font-bold ${highlighted ? 'text-white' : 'text-gray-800 dark:text-white'}`}>
            {price}
          </span>
          {price !== 'Free' && (
            <span className={`text-sm ${highlighted ? 'text-white opacity-90' : 'text-gray-500 dark:text-gray-400'}`}>
              /month
            </span>
          )}
        </div>
        <p
          className={`mb-6 ${
            highlighted ? 'text-white opacity-90' : 'text-gray-600 dark:text-gray-300'
          }`}
        >
          {description}
        </p>
